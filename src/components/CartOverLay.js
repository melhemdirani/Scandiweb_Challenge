...
class CartOverLay extends Component {
    ...
    calculateTotal = () => {
        let NewTotal = this.props.items.reduce(
            (accumalatedPrice, item) => 
                accumalatedPrice + item.quantity*item.price[this.props.currency].amount, 0
        )
    ...
    render() {
        const {currencies, currency, toggleCart, items, totalQuantity, total} = this.props
        return (
            <div className="CartOverLay_Container">
                <p className="CartOverLay_Title">
                    My Bag, 
                    <span className="CartOverLay_SubTitle">
                        {" "}{totalQuantity} item{items.length !== 1  && <span>s</span>}
                    </span> 
                </p>
                ...
                <div className="CartOverLay_SubContainer">
                        <div className="CartOverLay_Total">
                            ...
                            <p className="CartOverLay_Total_Price">{currencies[currency]}{total}</p>
                        </div>
                        <div className="CartOverLay_Button_Container">
                            <Link to="/bag">
                                <StyledCartButton location="view bag" onClick={toggleCart}>
                                    VIEW BAG
                                </StyledCartButton>
                            </Link>
